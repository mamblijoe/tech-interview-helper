...
//@ts-nocheck
import preset from "../../assets/presets/base.json";
import { useEffect, useState } from "react";

const Presets = () => {
  const [controlledQuestions, setControlledQuestions] = useState(preset);
  const [selectedGroup, setSelectedGroup] = useState<string[]>([]);
  const [open, setOpen] = useState(false);

  // console.log(
  //   JSON.stringify(preset.map((item, index) => ({ ...item, id: index + 1 }))),
  // );

  const resetData = () => {
    localStorage.removeItem("preset");
    localStorage.removeItem("group");
    setControlledQuestions(preset);
    setSelectedGroup([]);
  };
  ...
  const saveData = () => {
    localStorage.setItem("preset", JSON.stringify(controlledQuestions));
  ...
  const getGroupList = () => {
    return Array.from(new Set(preset.map((item) => item.group)));
  };
